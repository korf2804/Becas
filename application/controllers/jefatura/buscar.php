	...
	class Buscar extends CI_Controller{
		...
		public function busqueda_matricula(){
			if($this->session->userdata('logged_in')){
				...
				$datos = array(
	                    ...
	                    'departamento' => $session_data['departamento'],
	                    'anio' => date('Y'), 
	                  );
	          	...
	          	$this->load->view('front_end/jefatura/buscar_matricula');
		...
		public function busqueda_nombre(){
			if($this->session->userdata('logged_in')){
				...
				$datos = array(
	          	...
	          	$this->load->view('plantilla/front_end/header');
	          	$this->load->view('front_end/jefatura/nav_jefatura');
	          	$this->load->view('front_end/jefatura/buscar_nombre');
	          	$this->load->view('plantilla/front_end/footer');
	        }
	        else{
	        	redirect('error/error_401', 'refresh');
		...
		public function busqueda_apellidos(){
			if($this->session->userdata('logged_in')){
				...
				$datos = array(
	          	...
	          	$this->load->view('plantilla/front_end/header');
	          	$this->load->view('front_end/jefatura/nav_jefatura');
	          	$this->load->view('front_end/jefatura/buscar_apellido');
	          	$this->load->view('plantilla/front_end/footer');
	        }
	        else{
	        	redirect('error/error_401', 'refresh');
			...
			if($this->session->userdata('logged_in')){
				...
				$result = $this->Searchalum_model->consulta_matricula(trim($matricula), trim($carrera));
	         	...
	         	$this->load->view('plantilla/front_end/head', $datos);
	          	...
	          	$this->load->view('plantilla/front_end/header');
	          	$this->load->view('front_end/jefatura/nav_jefatura');
	          	$this->load->view('front_end/jefatura/resultado_busqueda');
	          	$this->load->view('plantilla/front_end/footer');
	        }
	        else{
	        	redirect('error/error_401', 'refresh');
		...
		public function resultado_busqueda_nombre()
		{
			if($this->session->userdata('logged_in')){
				...
				$carrera =  $this->security->xss_clean(strip_tags($this->input->post('carrera')));
				$result = $this->Searchalum_model->consulta_nombre(trim($nombre), trim($carrera));
	       		if($result != FALSE){
	         		$datos = array(
	         			'usuario' => $session_data['usuario'],
	         			'perfil' => $session_data['perfil'],
		...
		public function resultado_busqueda_apellido()
		{
			if($this->session->userdata('logged_in')){
				...
				$carrera =  $this->security->xss_clean(strip_tags($this->input->post('carrera')));
				$result = $this->Searchalum_model->consulta_apellido(trim($apellido), trim($carrera));
	       		if($result != FALSE){
	         		$datos = array(
	         			'usuario' => $session_data['usuario'],
	         			'perfil' => $session_data['perfil'],
